...
TransformPredictor::Config
TransformPredictor::readConfig(ros::NodeHandle nh, bool group_lengthscale,
                               bool group_signal_noise_sd) {
  Config config;
  ...
  nh.param("max_data_difference", config.max_data_difference, kDefaultMaxDataDifference);
  if (config.max_data_difference < 0) {
    throw std::runtime_error("Max data difference must be greater than or equal to 0");
  }
...
void TransformPredictor::addTransformMeasurement(
  ...
  for (size_t i = 0; i < transform_data_list_.size(); ++i) {
    Eigen::Matrix<double, 6, 1> log_transform =
        (transform_data_list_.back().transform.inverse() * transform_it->transform)
            .log();
  ...
  //go through and crop data to get rid of any jumps (such as an angle wrapping)
  for(int i = transform_data_list_.size() - 2; i >= 0; --i){
    for (const Eigen::VectorXd& relative_log_transform : relative_log_transforms) {
      double difference = std::abs(relative_log_transform[i+1] - relative_log_transform[i]);
...
void TransformPredictor::predictTransform(
  ...
  relative_transform_log->coeffRef(3) = rx_gp_.predict(relative_prediction_time);
  relative_transform_log->coeffRef(4) = ry_gp_.predict(relative_prediction_time);
  relative_transform_log->coeffRef(5) = rz_gp_.predict(relative_prediction_time);
}
...
void TransformPredictor::predictTransform(
  ...
  Eigen::Matrix<double, 6, 1> relative_transform_log;
  predictTransform(prediction_time, &relative_transform_log);
